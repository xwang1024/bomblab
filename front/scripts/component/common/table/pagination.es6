...
(function(window, document, $, module, exports, require){
  ...
  $('tfoot ul.pagination').addClass('pagination-sm');

  var $itemsPerPageSelector = $('#page-size-selector');
  var pageSize = $itemsPerPageSelector.data('pageSize');
  $itemsPerPageSelector.val(pageSize);
  $itemsPerPageSelector.on('change', function(event) {
    query.remove('page').set('pageSize', $itemsPerPageSelector.val());
    window.location.search = query.toString();
  });
