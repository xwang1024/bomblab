...
(function(window, document, $, module, exports, require, swal, Qiniu, QiniuConfig){
  ...
  let data = {};
  let _id = null;
  let $element = $('#detail-area>.panel');
  
  $('[name=groupDetailBtn]').click(function() {
    _id = $(this).parents('tr').data('id');
    Loader.show();
    $.ajax({
      url : '/api/admin/customGroupSend/' + _id,
      type : 'GET',
      dataType: 'json',
  ...
  function render() {
    $element.html(`
      <div class="panel-heading">
        <input name="name" class="form-control input-sm" value="${data.name}" style="display: inline-block; width: 90px;" placeholder="消息名称" autocomplete="off">
        <button class="btn btn-sm btn-default pull-right" name="cancelBtn">取消</button>
        <button class="btn btn-sm btn-green pull-right mr-sm" name="saveReplyBtn">保存</button>
        <button class="btn btn-sm btn-primary pull-right mr-sm" name="addImageBtn">增加图片</button>
        <button class="btn btn-sm btn-primary pull-right mr-sm" name="addTextBtn">增加文本</button>
      </div>
