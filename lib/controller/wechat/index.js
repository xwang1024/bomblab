  ...
  if(name === 'scan') {
    if(ticket) {
      InvitationCard.findOne({ qrTicket: ticket }).populate('invitationTask').exec((err, cardDoc) => {
        ...
        if(cardDoc && cardDoc.invitationTask.status === 'OPEN' && cardDoc.openId !== openId) {
          Subscriber.findOne({ openId, subscribe: true }).exec((err, subscriberDoc) => {
            if(err) return console.error(err);
            if(subscriberDoc) return Wechat.sendImage(openId, "你已经关注，不能被邀请");
          });
        }
      });
    }
      ...
      Wechat.getSubscriberInfo(openId).then((info) => {
        ...
        if(introducer) {
          ...
          if(card.invitationTask.invitedFeedback) {
            Wechat.sendText(introducer.openId, card.invitationTask.invitedFeedback.replace(/###/g, info.nickname));
          }
        }
