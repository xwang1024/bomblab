...
module.exports = {
  ...
  userType: function(userTypeCode) {
    var map = {'patient':'患者', 'sib':'家属'};
    if(userTypeCode) return map[userTypeCode];
  },
  noEnter: function(s) {
    return s.replace(/[\r\n]/g, "");
  },
  questionNoArray:function(questions, nos) {
    var origin = [];
    var result = [];
    for(var i in questions) {
      var question = questions[i];
      if(question.hasOptions) {
        origin.push(question.no);
      }
    }
    console.log(origin, nos)
    for(var i in origin) {
      if(nos.indexOf(origin[i]) >= 0) {
