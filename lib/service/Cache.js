...
exports.setEx = function setCache(k, v, exSeconds) {
  if(!k) throw new Error('[Cache:set] k is undefined');
  return new Promise((resolve, reject) => {
    let data = { value: v };
    redisClient.setex(prefix + k, exSeconds, JSON.stringify(data), (err, result) => {
      if(err) return reject(err);
...
exports.setNxEx = function (k, v, exSeconds) {
  if(!k) throw new Error('[Cache:setNxEx] k is undefined');
  return new Promise((resolve, reject) => {
    let data = { value: v };
    redisClient.set(prefix + k, JSON.stringify(data), 'NX', 'EX', exSeconds, (err, result) => {
      if(err) return reject(err);
      return resolve(result);
    });
  });
