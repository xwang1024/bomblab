...
exports.sendCard = function(openId) {
  ...
  const settings = {};
  const subscriber = {};
  const existedInvitationCard = {};
  const assetPath = {
    qrCode: path.resolve(__dirname, '../../static_files/qrCode/invitedBy-' + openId + '.png'),
    avatar: path.resolve(__dirname, '../../static_files/avatar/' + openId + '.png'),
    invitationCard: path.resolve(__dirname, '../../static_files/invitationCard/'+openId+'.png')
  };

  // 判断邀请功能状态
  workflow.on('checkFeatureStatus', () => {
    debug('Event: checkFeatureStatus');
    Setting.find({ key: { $in: ['invitationCardStatus', 'invitationCardImage'] } }).exec((err, docs) => {
      if(err) return workflow.emit('error', err); // 错误
      docs.forEach((doc) => {
        settings[doc.key] = doc.value;
        settings[doc.key].updatedAt = doc.updatedAt;
      });
      if(!settings.invitationCardStatus || !settings.invitationCardImage) return Wechat.sendText(openId, '邀请任务暂未开放~');
  ...
  workflow.on('getSubscriberInfo', () => {
    ...
    Wechat.getSubscriberInfo(openId).then((info) => {
      ...
      workflow.emit('checkCardStatus'); // 获取邀请卡记录
    }).catch((err) => {
  ...
  // 获取邀请卡记录
  workflow.on('checkCardStatus', () => {
    debug('Event: checkCardStatus');
    InvitationCard.findOne({ openId }).exec((err, doc) => {
      if(err) return workflow.emit('error', err);
      if(!doc) return workflow.emit('downloadQR'); // 申请QR，下载QR
      Object.assign(existedInvitationCard, doc.toJSON());
  ...
  workflow.on('downloadQR', () => {
    ...
    Wechat.getQRCodeTicket('invitedBy-' + openId).then((res) => {
      if(!res.ticket) return workflow.emit('error', new Error('未获得ticket: ' + res)); // 未得到 ticket
      Wechat.getQRCodeImage(res.ticket).then((qrReq) => {
  ...
  workflow.on('generateCard', () => {
    ...
    // 生成 cardSetting
    let cardSetting = {};
    settings.invitationCardImage.objects.forEach(function(o) {
    ...
    ctx.font = fontSize / cardSetting.background.scaleX + 'px Arial';
    ctx.fillStyle = textFill;
    ctx.fillText((subscriber.nickname || "").substring(0, 10), textLeft, textTop);
    ...
    invitationCardStream.on('finish', function () {
      InvitationCard.findOneAndUpdate({ openId }, { 
        openId: openId,
  ...
  workflow.on('isAvatarChanged', () => {
    ...
    console.log(subscriber.headimgurl == existedInvitationCard.headImgUrl)
    console.log(subscriber.headimgurl)
    console.log(existedInvitationCard.headImgUrl)
    if(subscriber.headimgurl == existedInvitationCard.headImgUrl) {
  ...
  workflow.on('isNicknameChanged', () => {
    ...
    if(subscriber.nickname == existedInvitationCard.nickname) {
  ...
  workflow.on('isSettingChanged', () => {
    ...
    if(settings.invitationCardImage.updatedAt <= existedInvitationCard.updatedAt) { // 配置较老
